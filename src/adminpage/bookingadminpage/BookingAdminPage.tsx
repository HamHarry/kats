...
import { approveBookingById, cancelBookingById, getAllBookingPaginations, isDeleteBookingById } from "../../stores/slices/bookingSlice";
...
const BookingAdminPage = () => {
  ...
  const fetchAllBooking = useCallback(async () => {
    try {
      ...
      const { data: bookingsRes = [] } = await dispath(getAllBookingPaginations({ term: searchTerm, receiptBookNo: selectedReceiptBookNo })).unwrap();
      setBookingDatas(bookingsRes.filter((item: BookingData) => item.delete === DeleteStatus.ISNOTDELETE));
    } catch (err) {
      console.log(err);
    } finally {
  ...
  const isStatusDelete = async () => {
    try {
      ...
      await dispath(isDeleteBookingById({ ...selectDataBooking, delete: DeleteStatus.ISDELETE })).unwrap();
  ...
  const onSubmit = async () => {
    try {
      ...
      await dispath(approveBookingById({ ...selectDataBooking, status })).unwrap();
    } catch (e) {
      console.log(e);
    } finally {
      setIsBookingLoading(false);
  ...
  const cancel = async () => {
    try {
      setIsBookingLoading(true);
      if (!selectDataBooking?._id) return;
      await dispath(cancelBookingById({ ...selectDataBooking, status: BookingStatus.CANCELED })).unwrap();
    } catch (e) {
      console.log(e);
    } finally {
      setIsBookingLoading(false);
  ...
  /* ── select menu ────────────────────────────────────── */
  const receiptBookNoList = bookingDataLites.map((b) => b.receiptBookNo).filter((v, i, s) => s.indexOf(v) === i);
    ...
    <div className="container-BookingAdmin">
      ...
      <div className="wrap-container-BookingAdmin">
        ...
        {bookingDatas.map((item, index) => {
          ...
          const isDone = item.status === BookingStatus.COMPLETED || item.status === BookingStatus.CANCELED;

          return (
            <div key={index} className={`grid-BookingAdmin ${isGun ? "type-gun" : "type-coat"}`} style={{ animationDelay: `${index * 40}ms` }}>
              {/* Card Header */}
              <div className="BookingAdmin-card-header">
                <div className="BookingAdmin-image">
                  <img src={item.product.name === "KATS Coating" ? "/assets/logokats.jpg" : "/assets/logoGun.jpg"} alt="product" />
                </div>

                <div className="BookingAdmin-header-meta">
                  <p className="BookingAdmin-product-name">{item.product.name}</p>
              ...
              <div className="BookingAdmin-content">
                <div className="info-grid">
                  ...
                  <div className="info-item full">
                    ...
                    <span className={`info-value accent ${isGun ? "gun" : "coat"}`}>
                      {item.product.name} — {item.price.amount} {t("บาท")}
                    </span>
                  </div>

                  <div className="info-divider" style={{ gridColumn: "1 / -1" }} />
              ...
              <div className="BookingAdmin-card-footer">
                <div className="footer-actions">
                  ...
                  <Tooltip title="แก้ไขข้อมูล">
                    <button className="icon-btn edit" onClick={() => item._id && navigate(`/admin/booking/edit/${item._id}`)}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {/* ── Dialogs ──────────────────────────────────────── */}
      {/* Pay Image */}
      <Modal centered className="wrap-container-DialogPay" open={openDialogPay} onOk={() => setOpenDialogPay(false)} onCancel={() => setOpenDialogPay(false)}>
        <div className="ImagePay">
          <img src={getImagePath("booking", userInfo?.dbname, selectImagePay)} alt="slip" />
        </div>
      </Modal>
      ...
      <Modal centered className="wrap-container-DialogDelete" open={openDialogConfirmDelete} onCancel={() => setOpenDialogConfirmDelete(false)}>
        ...
        <div className="btn-DialogDelete-Navbar">
          ...
          <button onClick={() => setOpenDialogConfirmDelete(false)}>{t("ยกเลิก")}</button>
        </div>
      </Modal>

      {/* Confirm Approve */}
      <Modal centered className="wrap-container-DialogApprove" open={openDialogConfirmApprove} onCancel={() => setOpenDialogConfirmApprove(false)}>
        ...
        <div className="btn-DialogApprove-Navbar">
          ...
          <button onClick={() => setOpenDialogConfirmApprove(false)}>{t("ยกเลิก")}</button>
        </div>
      </Modal>

      {/* Confirm Cancel */}
      <Modal centered className="wrap-container-DialogApprove" open={openDialogCancelApprove} onCancel={() => setOpenDialogCancelApprove(false)}>
        ...
        <div className="btn-DialogApprove-Navbar">
          ...
          <button onClick={() => setOpenDialogCancelApprove(false)}>{t("ยกเลิก")}</button>
        </div>
      </Modal>
